# lg-pdf-agent.py  LannGraph PDF Agent to review and summarize arXiv papers
## code generated by ChatGPT 4o and GHCP/4o
'''
...
- generate a markdown list of papers. Instruct the model to output the following in strict JSON format:
    ...
    - relevant tags.
'''
# before running this script make sure the server is listening for api calls and the model is loaded.
...
openai.api_key = "gimmedatb"  # dummy, not used
openai.api_base =  "http://127.0.0.1:5000/v1"  # langgraph appends /chat/completions
openai.api_type = "openai"

llm = ChatOpenAI(
    openai_api_base=openai.api_base,
    openai_api_key=openai.api_key,
    model="qwen2.5-3b-instruct-q8_0.gguf",  # same model string ooba displays
    temperature=0.3,
)
...
def analyze_papers_node(state):
    ...
    for paper in state["papers"]:
        ...
        try:
            # models may return imperfect JSON, clean it as needed
            # in the case of qwen, it returns a JSON block with ```json\n prefix. we check for a suffix just in case.
